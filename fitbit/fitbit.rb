...
def write_csvs(opts = {})
  ...
  filestring = CSV.generate do |csv|
    ...
    (start_date..end_date).each do |date|
      ...
      file = "#{dir}/#{filename}"
      weight_hash = JSON.parse(File.read(file))
      date = date_from_filename(file)

      lean_mass = (100 - weight_hash["body"]["fat"].to_f)/100 * weight_hash["body"]["weight"].to_f
      fat_pounds = (weight_hash["body"]["fat"].to_f/100.0) * weight_hash["body"]["weight"].to_f


      # SLEEP
      filename = filename_for_date(date, "sleep")
      file = "#{dir}/#{filename}"
      sleep_hash = JSON.parse(File.read(file))
      date = date_from_filename(file)

      total_minutes = sleep_hash["summary"]["totalMinutesAsleep"].to_i

      bed_at = sleep_hash["sleep"].first.try(:[], "startTime").to_s.scan(/T.*/).last.to_s.gsub("T", "").gsub(/\..*/, "")
      slept_at = fell_asleep_at sleep_hash["sleep"]
      ...
      file = "#{dir}/#{filename}"
      step_hash = JSON.parse(File.read(file))
      date = date_from_filename(file)


      line = [
        pretty_date,
        weight_hash["body"]["weight"],
        weight_hash["body"]["fat"],
        lean_mass.round(1),
        fat_pounds.round(1),
        ...
        step_hash["summary"]["steps"],
        step_hash["summary"]["caloriesOut"],

        step_hash["summary"]["sedentaryMinutes"],
        step_hash["summary"]["lightlyActiveMinutes"],
        step_hash["summary"]["fairlyActiveMinutes"],
        step_hash["summary"]["veryActiveMinutes"]
      ]
